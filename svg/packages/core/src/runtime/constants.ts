...
export const FIELDS = [
  ["Basics", [
    { key: "tagName", label: "Element type", kind: "readonly", value: (node: Element) => node.tagName.toLowerCase() },
...
export const FIELD_MAP: Map<string, any> = new Map(
  FIELDS.flatMap(([, fields]) => fields.map((field) => [field.key, field]))
);
...
export const GRID_SNAP_DEFAULT_SIZE = 10;
