...
const EDITABLE_NODE_BASE_SCORE = {
  circle: 500,
  ellipse: 500,
  line: 500,
  path: 500,
...
function getChildSvgElements(node: Element): EditorSvgElement[] {
  return Array.from(node.children).filter((child): child is EditorSvgElement => child instanceof SVGElement);
}

function getDescendantSvgElements(node: ParentNode): EditorSvgElement[] {
  return Array.from(node.querySelectorAll("*")).filter((child): child is EditorSvgElement => child instanceof SVGElement);
}

export function createSvgMetadataTools(state: SvgRuntimeStateLike): MetadataTools {
  function getNodeArea(node: Element | null | undefined) {
    try {
    ...
    try {
      const box = (node as SVGGraphicsElement | null | undefined)?.getBBox?.();
  ...
  function getEditableNodeScore(node: Element | null | undefined) {
    if (!node?.tagName) {
      return -1;
    }

    const tag = node.tagName.toLowerCase() as keyof typeof EDITABLE_NODE_BASE_SCORE;
    return (EDITABLE_NODE_BASE_SCORE[tag] || 0) + getNodeArea(node);
  }

  function findEditableAncestor(startNode: Element | null | undefined, stopNode: Element | null = null) {
    ...
    while (current) {
      ...
      if (current.tagName.toLowerCase() === "tspan") {
        ...
        if (parentText instanceof SVGElement && parentText.dataset.editorId) {
          return parentText;
        }
      }

      if (current instanceof SVGElement && current.dataset.editorId && EDITABLE_TAGS.has(current.tagName.toLowerCase()) && !isNodeHidden(current)) {
        return current;
      }
  ...
  function getPrimaryEditableDescendant(node: Element | null | undefined, preferredNode: Element | null = null) {
    ...
    const candidates = getDescendantSvgElements(node)
  ...
  function getRenderableChildren(node: Element) {
    return getChildSvgElements(node).filter((child) => child.tagName.toLowerCase() !== "style");
  }

  function buildNodeKey(node: Element | null | undefined) {
    if (!node) {
      return null;
    }
    ...
    while (current && current !== state.svgRoot) {
      ...
      const index = siblings.indexOf(current as EditorSvgElement);
  ...
  function addEditorIds(root: Element) {
    for (const node of [root, ...getDescendantSvgElements(root)]) {
  ...
  function rebuildNodeMap() {
    ...
    for (const node of [state.svgRoot, ...getDescendantSvgElements(state.svgRoot)]) {
  ...
  function getNodeKey(node: Element | null | undefined) {
    return (node ? state.nodeKeyByNode.get(node) : null) || buildNodeKey(node);
  }

  function getNodeKeyByEditorId(editorId: string) {
    return state.nodeKeyByEditorId.get(editorId) || null;
  }

  function getEditorIdByNodeKey(nodeKey: string) {
  ...
  function labelFor(node: Element) {
  ...
  function resolveEditableNode(node: Element | null | undefined, options: ResolveEditableNodeOptions = {}) {
    if (!node) {
      return null;
    }
  ...
  function resolveSelectionEditorId(editorId: string, options: ResolveEditableNodeOptions = {}) {
    const node = state.nodeMap.get(editorId);
    const resolvedNode = resolveEditableNode(node, options);
    return resolvedNode instanceof SVGElement && resolvedNode.dataset.editorId ? resolvedNode.dataset.editorId : editorId;
  }

  function isNodeLocked(node: Element | null | undefined) {
    const nodeKey = getNodeKey(node);
    return Boolean(nodeKey && state.lockedNodeKeys.has(nodeKey));
  }

  function isNodeHidden(node: Element | null | undefined) {
    return node?.getAttribute("display") === "none";
  }

  function getZOrder(node: Element | null | undefined) {
    if (!node || node === state.svgRoot || !node.parentElement) {
      return "";
    }

    const siblings = getRenderableChildren(node.parentElement);
    const index = siblings.indexOf(node as EditorSvgElement);
    return index >= 0 ? String(index) : "";
  }

  function setZOrder(node: Element | null | undefined, requestedOrder: string | number) {
    if (!node || node === state.svgRoot || !node.parentElement) {
      return false;
    }
    ...
    const siblings = getRenderableChildren(parent);
    const currentIndex = siblings.indexOf(node as EditorSvgElement);
    if (currentIndex < 0) {
      return false;
    }
    ...
    reordered.splice(targetIndex, 0, node as EditorSvgElement);
    const anchor = reordered[targetIndex + 1] || null;
    parent.insertBefore(node, anchor);
    return true;
  }

  function visibleField(node: Element, field: FieldDefinition) {
