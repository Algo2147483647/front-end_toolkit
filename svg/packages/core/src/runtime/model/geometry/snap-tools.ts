import { GRID_SCREEN_SIZE } from "../../constants";
import type { GridMetrics, SnapTools, SvgRuntimeStateLike, ViewportTools } from "../types";
...
export function createSvgSnapTools({ state, getNumericAttr, roundCoordinate }: SnapDeps): SnapTools {
  ...
  function snapCoordinate(value: number, step: number, origin: number) {
  ...
  function snapNodeToGrid(node: Element | null | undefined) {
  ...
  function snapFieldValue(fieldKey: string, value: string) {
