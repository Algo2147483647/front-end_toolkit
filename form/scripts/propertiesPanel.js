...
function renderProperties() {
  ...
  let propertiesHTML = `
    <div class="property-field">
      ...
      <input class="ant-input" value="${escapeAttribute(comp.type)}" disabled>
    </div>
    <div class="property-field">
      <label class="property-label">Field Name</label>
      <input class="ant-input" value="${escapeAttribute(comp.config.name)}"
             onchange="updateProperty('name', this.value)">
    </div>
    <div class="property-field">
      <label class="property-label">Display Title</label>
      <input class="ant-input" value="${escapeAttribute(comp.config.title)}"
    ...
    propertiesHTML += `
      ...
      <div class="property-field vertical">
        ...
        <textarea class="ant-input" style="height: 120px;"
                  onchange="updateCollapsePanels(this.value, '${comp.id}')">${escapeHTML(JSON.stringify(panels, null, 2))}</textarea>
  ...
  if (comp.type === 'Cascader') {
    propertiesHTML += `
      <div class="property-field vertical">
        ...
        <textarea class="ant-input" style="height: 120px;"
                  onchange="updateCascaderOptions(this.value, '${comp.id}')">${escapeHTML(JSON.stringify(comp.config.options || [], null, 2))}</textarea>
        <p style="font-size: 12px; color: #999; margin-top: 4px;">Enter in JSON format</p>
      </div>`;
  }
  ...
  if (comp.type === 'Divider') {
    propertiesHTML += `
      <div class="property-field">
        ...
        <input class="ant-input" value="${escapeAttribute(comp.config.content || '')}"
  ...
  if (comp.type === 'Grid') {
    propertiesHTML += `
      <div class="property-field">
        ...
        <input type="number" class="ant-input" min="1" max="12" value="${escapeAttribute(comp.config.columns || 3)}"
               onchange="updateProperty('columns', parseInt(this.value, 10))">
      </div>`;
  }
  ...
  if (comp.type === 'ColorPicker') {
    propertiesHTML += `
      <div class="property-field">
        ...
        <input type="color" class="ant-input" value="${escapeAttribute(comp.config.defaultValue || '#1890ff')}"
               onchange="updateProperty('defaultValue', this.value)">
        <input class="ant-input" value="${escapeAttribute(comp.config.defaultValue || '#1890ff')}"
  ...
  if (comp.type === 'Slider') {
    propertiesHTML += `
      <div class="property-field">
        ...
        <input type="number" class="ant-input" value="${escapeAttribute(comp.config.min || 0)}"
               onchange="updateProperty('min', parseInt(this.value, 10))">
      </div>
      <div class="property-field">
        <label class="property-label">Max Value</label>
        <input type="number" class="ant-input" value="${escapeAttribute(comp.config.max || 100)}"
               onchange="updateProperty('max', parseInt(this.value, 10))">
      </div>
      <div class="property-field">
        <label class="property-label">Default Value</label>
        <input type="number" class="ant-input" value="${escapeAttribute(comp.config.defaultValue || 0)}"
               onchange="updateProperty('defaultValue', parseInt(this.value, 10))">
      </div>`;
  }
  ...
  if (comp.type !== 'Divider' && comp.type !== 'Card' && comp.type !== 'Grid') {
    propertiesHTML += `
      ...
      <div class="property-field">
        <label class="property-label">Default Value</label>
        <input class="ant-input" value="${escapeAttribute(comp.config.defaultValue || '')}"
               onchange="updateProperty('defaultValue', this.value)">
      </div>`;
  }
