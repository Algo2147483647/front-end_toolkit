...
export default function SaveJsonModal({
  ...
  const normalizedFileName = ensureJsonExtension(sourceFileName || "global-asset-net-worth-board.snapshot.json");
  const newFileName = buildTimestampFileName(normalizedFileName);

  return (
    <div
      className="save-json-modal is-visible"
      aria-hidden="false"
      onClick={(event) => event.target === event.currentTarget && onClose()}
    >
      <div className="save-json-dialog" role="dialog" aria-modal="true" aria-labelledby="save-json-title">
        <h3 id="save-json-title">Save Snapshot JSON</h3>
        <p className="save-json-description">
          {canOverwrite
        ...
        <div className="save-json-actions">
          <button
            ...
            title={canOverwrite ? `Overwrite ${normalizedFileName}` : "Open the JSON with file access to enable overwrite."}
            onClick={onOverwrite}
          >
            Overwrite Original
          </button>
          <button className="ghost-btn" type="button" onClick={onSaveNew}>
            Save New Copy
          </button>
