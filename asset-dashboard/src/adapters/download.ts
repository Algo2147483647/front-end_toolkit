...
export function ensureJsonExtension(fileName: string): string {
  const name = String(fileName || "snapshot").trim() || "snapshot";
  return /\.json$/i.test(name) ? name : `${name}.json`;
}

export function buildTimestampFileName(fileName: string, date = new Date()): string {
  const normalizedFileName = ensureJsonExtension(fileName);
  const match = normalizedFileName.match(/^(.*?)(\.json)$/i);
  const baseName = match ? match[1] : normalizedFileName;
